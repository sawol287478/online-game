/* =========================================================================
   ...
   (() => {
    ...
    class Board {
      ...
      fillInitialRows(rng) {
        ...
        for (let r = 0; r < ROWS; r++) {
          // 위쪽 절반은 비워두고 시작 (너무 급작스럽지 않게)
          if (r < ROWS - 4) this.grid.push(new Array(COLS).fill(0));
          else this.grid.push(generateRow(rng, this.round++));
        }
      }
    }
    ...
    function beginMatchFlow() {
      ...
      state.opp.grid = [];
      for (let r = 0; r < ROWS; r++) {
        if (r < ROWS - 4) state.opp.grid.push(new Array(COLS).fill(0));
        else state.opp.grid.push(generateRow(rngCopy, tmp.round++));
      }
    ...
    function resolveShot(board) {
      ...
      // 한 줄씩 내리고 새 줄 생성
      board.grid.shift();
      board.grid.push(generateRow(state.rng, board.round++));
